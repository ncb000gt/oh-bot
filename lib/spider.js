...
function Spider(opts) {
  ...
  this.urls = redback.createHash("urls");
  this.started = false;
...
Spider.prototype.handleLink = function(link, opts) {
  ...
  //might have to do something here to maintain data safety...unsure as of yet.

  self.urls.exists(link, function(err, exists) {
    if (!exists) {
      var url = {
        status: opts.status,
        from: {}
      ...
      url.from[opts.path] = '';
      self.urls.set(link, url, function(err) {
        console.log('set new url');
      });
    } else {
      self.urls.get(link, function(err, url) {
        url.from[opts.path] = '';
        self.urls.set(link, url, function(err) {
          console.log('set existing url');
        });
      });
    }
  });

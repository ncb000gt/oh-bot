var emitter = require('events').EventEmitter,
    ...
    db = require('riak-js').getClient({debug: false, port: 8091}),
    http = require('http');
...
function Spider(opts) {
  ...
  setInterval(function() {
    ...
    if (self.get_queue.length > 0 && self._concurrent_gets <= self.concurrent_gets) {
      ...
      self.emit('done', self.already_processed);
...
Spider.prototype.handleLink = function(link, opts) {
  ...
  db.get("urls", encodeURIComponent(link), function(err, url, meta) {
    console.log(err);
    console.log(url);
    console.log(meta);
    if (!url || err && err.statusCode === 404) {
      url = {
        status: opts.status,
        from: {}
      };
    ...
    if (!(opts.path in url.from)) {
      url.from[opts.path] = '';

      db.save("urls", encodeURIComponent(link), url, meta, function(err) {
